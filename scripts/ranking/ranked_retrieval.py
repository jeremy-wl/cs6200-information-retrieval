...
from use_index import UseIndex
...
class RankedRetrieval(object):
    ...
    def tf_idf_query(self, query):
        terms = data_transformer.extract_terms_from_query(query)
...
def run_ranked_retrieval(index_folder_name, content_folder_name, queries_file, k):
    ...
    for raw_query in raw_queries:
        terms = data_transformer.extract_terms_from_query(raw_query)
        query = ' '.join(terms)
        output[raw_query] = [query, rr.get_top_k_docs(raw_query, k)]

    with open('../../outputs/query/Output.json', 'w') as output_file:
        output_file.write(json.dumps(output, indent=2))


if __name__ == '__main__':
    index_folder_name = '../../outputs/index'
    content_folder_name = '../../outputs/html'
    queries_file = '../../outputs/query/Queries.txt'
    k = 5

    run_ranked_retrieval(index_folder_name, content_folder_name, queries_file, k)
